...
namespace MathNet.Symbolics.Interpreter
{
    ...
    /// <summary>
    /// Expression Stack with auto expansion.
    /// </summary>
    public sealed class SignalStack
    {
        private Signal[] stack;
        private int count;
        private int maxSize;

        public SignalStack(int minSize, int maxSize)
        {
        ...
        private void Expand()
        {
            if(maxSize > 0 && stack.Length * 2 > maxSize)
                throw new MathNet.Symbolics.Exceptions.ParsingException("Parsing failed. Maximum expression stack size exceeded.");
            Signal[] newStack = new Signal[stack.Length * 2];
