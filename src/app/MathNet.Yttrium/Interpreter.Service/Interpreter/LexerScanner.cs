...
namespace MathNet.Symbolics.Interpreter
{
    public sealed class LexerScanner
    {
        ...
        private readonly NumberFormatInfo numberFormat = Config.InternalNumberFormat;
        ...
        public LexerToken NextToken()
        {
            ...
            throw new MathNet.Symbolics.Exceptions.ParsingException("Parsing failed. Lexer Scanner detected unexpected token '" + beginChar + "'");
        }
        ...
        private LexerToken ScanLiteral()
        {
            ...
            while(true)
            {
                ...
                if(c == eof)
                    throw new MathNet.Symbolics.Exceptions.ParsingException("Parsing failed. Unexpected end of file. Did you forget to close a literal encapsulation?");
                sb.Append(c);
                lexer.Consume();
            }
            lexer.Consume();
        ...
        private LexerToken ScanSymbolIdentifier()
        {
            ...
            while(IsSymbolIdentifierCharacter(lexer.LookaheadFistCharacter)
                && library.ContainsEntity(sb.ToString() + lexer.LookaheadFistCharacter))
            {
                sb.Append(lexer.LookaheadFistCharacter);
                lexer.Consume();
            }
