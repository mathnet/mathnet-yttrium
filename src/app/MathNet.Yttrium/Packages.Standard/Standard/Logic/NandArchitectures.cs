...
{
    public class NandArchitectures : GenericArchitectureFactory
    {
        private static readonly MathIdentifier _entityId = new MathIdentifier("Nand", "Std");
        public static MathIdentifier EntityIdentifier
        {
            get { return _entityId; }
        }

        public NandArchitectures()
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Logic"),
                LogicValueCategory.IsLogicValueMember,
                delegate(Port port) { return new ProcessBase[] { LogicValue.CreateNandProcess(0, port.InputSignalCount - 1, 0) }; });
        }
    }
}
