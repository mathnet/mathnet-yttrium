...
{
    public class OrArchitectures : GenericArchitectureFactory
    {
        private static readonly MathIdentifier _entityId = new MathIdentifier("Or", "Std");
        public static MathIdentifier EntityIdentifier
        {
            get { return _entityId; }
        }

        public OrArchitectures()
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Logic"),
                LogicValueCategory.IsLogicValueMember,
                delegate(Port port) { return new ProcessBase[] { LogicValue.CreateOrProcess(0, port.InputSignalCount - 1, 0) }; });
        }
    }
}
