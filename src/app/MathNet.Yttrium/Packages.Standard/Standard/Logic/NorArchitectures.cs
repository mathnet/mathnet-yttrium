...
{
    public class NorArchitectures : GenericArchitectureFactory
    {
        private static readonly MathIdentifier _entityId = new MathIdentifier("Nor", "Std");
        public static MathIdentifier EntityIdentifier
        {
            get { return _entityId; }
        }

        public NorArchitectures()
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Logic"),
                LogicValueCategory.IsLogicValueMember,
                delegate(Port port) { return new ProcessBase[] { LogicValue.CreateNorProcess(0, port.InputSignalCount - 1, 0) }; });
        }
    }
}
