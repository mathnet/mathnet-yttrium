...
{
    public class ExponentialArchitectures : GenericArchitectureFactory
    {
        private static readonly MathIdentifier _entityId = new MathIdentifier("Exponential", "Std");
        public static MathIdentifier EntityIdentifier
        {
            get { return _entityId; }
        }

        public ExponentialArchitectures()
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Real"),
                RealValueCategory.IsRealValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<RealValue, RealValue>(delegate(RealValue value) { return value.Exponential(); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Complex"),
                ComplexValueCategory.IsComplexValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<ComplexValue, ComplexValue>(ComplexValue.Exponential, ComplexValue.ConvertFrom, port.InputSignalCount) }; });
        }
            ...
            Analysis.DerivativeTransformation.Provider.Add(
                ...
                delegate(Port port, SignalSet manipulatedInputs, Signal variable, bool hasManipulatedInputs)
                {
                    ...
                    for(int i = 0; i < manipulatedInputs.Count; i++)
                        outputs.Add(manipulatedInputs[i] * port.OutputSignals[i]);
                    return outputs;
                }));
        }
    }
