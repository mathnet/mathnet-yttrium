...
{
    public class InvertArchitectures : GenericArchitectureFactory
    {
        private static readonly MathIdentifier _entityId = new MathIdentifier("Invert", "Std");
        public static MathIdentifier EntityIdentifier
        {
            get { return _entityId; }
        }

        public InvertArchitectures()
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Integer"),
                IntegerValueCategory.IsIntegerValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<IntegerValue, RationalValue>(delegate(IntegerValue value) { return value.Invert(); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Rational"),
                RationalValueCategory.IsRationalValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<RationalValue, RationalValue>(delegate(RationalValue value) { return value.Invert(); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Real"),
                RealValueCategory.IsRealValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<RealValue, RealValue>(delegate(RealValue value) { return value.Invert(); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Complex"),
                ComplexValueCategory.IsComplexValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<ComplexValue, ComplexValue>(ComplexValue.Invert, ComplexValue.ConvertFrom, port.InputSignalCount) }; });
        }
