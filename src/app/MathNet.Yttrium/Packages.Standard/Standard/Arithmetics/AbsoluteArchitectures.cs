    ...
    public class AbsoluteArchitectures : GenericArchitectureFactory
    {
            ...
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Integer"),
                ...
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<IntegerValue, IntegerValue>(delegate(IntegerValue value) { return value.Absolute(); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Rational"),
                RationalValueCategory.IsRationalValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<RationalValue, RationalValue>(delegate(RationalValue value) { return value.Absolute(); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Real"),
                RealValueCategory.IsRealValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<RealValue, RealValue>(delegate(RealValue value) { return value.Absolute(); }, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Complex"),
                ComplexValueCategory.IsComplexValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<ComplexValue, RealValue>(ComplexValue.Absolute, ComplexValue.ConvertFrom, port.InputSignalCount) }; });
        }
