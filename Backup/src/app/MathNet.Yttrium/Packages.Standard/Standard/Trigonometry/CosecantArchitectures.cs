    ...
    public class CosecantArchitectures : GenericArchitectureFactory
    {
            ...
            : base(_entityId)
        {
            AddArchitecture(EntityId.DerivePrefix("Real"),
                ...
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<RealValue, RealValue>(delegate(RealValue value) { return value.Cosecant(); }, RealValue.ConvertFrom, port.InputSignalCount) }; });

            AddArchitecture(EntityId.DerivePrefix("Complex"),
                ComplexValueCategory.IsComplexValueMember,
                delegate(Port port) { return new ProcessBase[] { new GenericStdParallelProcess<ComplexValue, ComplexValue>(delegate(ComplexValue value) { return value.Cosecant(); }, ComplexValue.ConvertFrom, port.InputSignalCount) }; });
        }
